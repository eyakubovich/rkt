...
func validateRoutes(t *testing.T, routes []ipam.Route) {
	expected := []ipam.Route{
		ipam.Route{
			Dst: net.IPNet{
	...
	for i := 0; i < len(routes); i++ {
		...
		if !a.Gateway.Equal(e.Gateway) {
			t.Errorf("route.Gateway mismatch: expected %v, got %v", e.Gateway, a.Gateway)
		}
	}
}
...
func TestParseRoutes(t *testing.T) {
	...
	opts[dhcpOptionStaticRoute] = []byte{10, 0, 0, 0, 10, 1, 2, 3, 192, 168, 1, 0, 192, 168, 2, 3}
...
func TestParseCIDRRoutes(t *testing.T) {
	opts := make(dhcp4.Options)
	opts[dhcpOptionClasslessStaticRoute] = []byte{8, 10, 10, 1, 2, 3, 24, 192, 168, 1, 192, 168, 2, 3}
